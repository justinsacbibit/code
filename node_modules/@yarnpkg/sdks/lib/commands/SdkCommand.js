...
class SdkCommand extends clipanion_1.Command {
    constructor() {
        ...
        this.integrations = clipanion_1.Option.Rest();
    }
    async execute() {
        ...
        while (nextProjectRoot !== currProjectRoot) {
            ...
            if (fslib_1.xfs.existsSync(fslib_1.ppath.join(currProjectRoot, fslib_1.Filename.pnpCjs))) {
                pnpFilename = fslib_1.Filename.pnpCjs;
                break;
            }
            // TODO: Drop support for .pnp.js files after they stop being used.
            if (fslib_1.xfs.existsSync(fslib_1.ppath.join(currProjectRoot, fslib_1.Filename.pnpJs))) {
                pnpFilename = fslib_1.Filename.pnpJs;
                break;
            }
        }
        ...
        const pnpPath = fslib_1.ppath.join(currProjectRoot, pnpFilename);
        const pnpApi = (0, dynamicRequire_1.dynamicRequire)(fslib_1.npath.fromPortablePath(pnpPath));
        // Need to setup the fs patch so we can read from the archives
        pnpApi === null || pnpApi === void 0 ? void 0 : pnpApi.setup();
        ...
        (0, generateSdk_1.validateIntegrations)(requestedIntegrations);
        ...
        if (allIntegrations.size === 0 && !onlyBase)
            throw new clipanion_1.UsageError(`No integrations have been provided as arguments, and no preexisting integrations could be found. Make sure to run \`yarn sdks <integrations>\` first, or \`yarn sdks base\` if you only need the SDK files and prefer to manage your own environment settings. Run \`yarn sdks -h\` to see the list of supported integrations.`);
        const report = await core_1.StreamReport.start({
            configuration,
            includeFooter: false,
            stdout: this.context.stdout,
        }, async (report) => {
            await (0, generateSdk_1.generateSdk)(pnpApi, {
                requestedIntegrations,
                preexistingIntegrations,
            }, {
                report,
