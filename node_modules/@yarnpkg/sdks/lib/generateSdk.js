...
exports.generateSdk = exports.Wrapper = exports.IntegrationsFile = exports.validateIntegrations = exports.getDisplayName = exports.SUPPORTED_INTEGRATIONS = exports.INTEGRATIONS_FILE = exports.SDK_FOLDER = void 0;
...
const getDisplayName = (name) => (0, startCase_1.default)(name).split(` `).map(word => (0, capitalize_1.default)(word)).join(` `);
...
const validateIntegrations = (integrations) => {
    ...
    if (unsupportedIntegrations.length > 0) {
        throw new clipanion_1.UsageError(`No supported integrations with the following names could be found: ${unsupportedIntegrations.join(`, `)}. Run \`yarn sdks -h\` to see the list of supported integrations.`);
    }
};
...
class IntegrationsFile {
    ...
    load(data) {
        ...
        if (Array.isArray(data.integrations)) {
            ...
            (0, exports.validateIntegrations)(this.integrations);
        }
    }
    exportTo(data) {
    ...
    async persist(dir) {
        ...
        content += (0, parsers_1.stringifySyml)(data);
...
class Wrapper {
    ...
    async writeManifest() {
        ...
        const manifest = (0, dynamicRequire_1.dynamicRequire)(fslib_1.npath.join(pkgInformation.packageLocation, `package.json`));
    ...
    async writeFile(relPackagePath, options = {}) {
        ...
        await fslib_1.xfs.mkdirPromise(fslib_1.ppath.dirname(absWrapperPath), { recursive: true });
        await fslib_1.xfs.writeFilePromise(absWrapperPath, TEMPLATE(relPnpApiPath, fslib_1.ppath.join(this.name, (_a = options.requirePath) !== null && _a !== void 0 ? _a : relPackagePath), options), {
            mode: options.mode,
        });
...
const generateSdk = async (pnpApi, { requestedIntegrations, preexistingIntegrations }, { report, onlyBase, verbose, configuration }) => {
    ...
    await report.startTimerPromise(`Generating SDKs inside ${core_1.formatUtils.pretty(configuration, exports.SDK_FOLDER, core_1.formatUtils.Type.PATH)}`, async () => {
        ...
        for (const sdks of integrationSdks) {
            ...
            await (generateDefaultWrapper === null || generateDefaultWrapper === void 0 ? void 0 : generateDefaultWrapper(pnpApi, targetFolder));
        }
        for (const [pkgName, generateBaseWrapper] of base_1.BASE_SDKS) {
            const displayName = (0, exports.getDisplayName)(pkgName);
        ...
        await report.startTimerPromise(`Generating settings`, async () => {
            for (const integration of allIntegrations) {
                if (preexistingIntegrations.has(integration)) {
                    report.reportInfo(core_1.MessageName.UNNAMED, `${Mark.Check} ${(0, exports.getDisplayName)(integration)} (updated 🔼)`);
                }
                else {
                    report.reportInfo(core_1.MessageName.UNNAMED, `${Mark.Check} ${(0, exports.getDisplayName)(integration)} (new ✨)`);
                }
            }
        });
    }
